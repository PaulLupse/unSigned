...
import {get_form} from "../user/back-end-connection";
import {use_key} from "./back-end-connection";
...
function KeyInputComponent() {
    ...
    const onSubmit:SubmitHandler<KeyFormInput> = async ({key}:KeyFormInput):Promise<void>=>{
        ...
        if(form) {
            navigate("/complete-form/form", {state:{form:form}});
        }
    }
...
function Base() {
    ...
    return (
        <>
            ...
            <div id={"Toata pagina"} style={{height:'100vh', display:"flex", flexDirection:'column', justifyContent:'center'}}>
                <Outlet />
            </div>
        </>
